...
For notation with subscripts, superscripts, primes, or stars, treat the complete token as one semantic object. Tokens such as `S_t`, `S_{t+1}`, `A_t`, `R_{t+1}`, `G_t`, `S_1/S_2/S_3`, and `\pi^*` must use one independent LaTeX/SVG object or a deliberately wide single-line inline zone. Do not rely on Unicode subscripts, accidental font glyphs, or a narrow mixed-text box. When a diagram node combines prose and a variable, use an inline or stacked composite inside the same bounded node; keep repeated tokens on a shared baseline, visual size, and aspect ratio.
...
6. Build diagrams as native objects. Use independent shapes, connectors, arrows, tables, and charts. Keep a label next to the object it describes, route connectors behind nodes or around text, and group semantically related objects when the API supports grouping. Prefer straight routes for the primary flow and reserve a curved route for an intentional return loop. Connector labels should sit beside or above the connector they explain, remain visually attached to it, and normally be at least 12–14 pt; never hide a key action label in a crowded corner or in the connector path. Do not flatten a diagram or an entire slide into an image.
7. Treat formulas as designed objects. Classify source and display role before repair. In the academic/scientific profile, use the controlled remote LaTeX → sanitized SVG path for all standalone/display and nontrivial equations when the user has authorized equation-source upload; do not try to imitate a fraction, integral, expectation, superscript, subscript, or argmax with scattered text fragments. Use an Office equation object only when the active runtime actually supports and preserves it and the user explicitly prioritizes character editability. Otherwise insert one independent SVG with preserved aspect ratio, tight bounds, a canonical source string, and a clear visual owner. For small variables embedded in diagrams, apply the complete-token rule above and leave deliberate breathing room around the token. Never split one equation into scattered text boxes or allow a formula to wrap silently.
...
- Never use a narrow ordinary text box for a meaningful state/action/reward/value token. If a token has a subscript, superscript, prime, or star, its notation must stay on one deliberate visual line and be legible at presentation scale.
- Connector labels are part of the diagram, not spare annotation. They need an obvious connector owner, a readable size, and enough clearance from nodes and neighboring labels.
- If one slide exposes a layout defect, sweep the same component pattern across the whole deck (all state nodes, action labels, formula pills, and return arrows), not just the named slide.
